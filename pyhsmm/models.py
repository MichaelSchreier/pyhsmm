...
from pyhsmm.basic.abstractions import (
    Model,
    ModelGibbsSampling,
    ModelEM,
    ModelMAPEM,
...
class _HMMBase(Model):
    ...
    def __init__(
        ...
        if init_state_distn is not None:
            ...
            self.init_state_distn = self._init_state_class(
                model=self, init_state_concentration=init_state_concentration, pi_0=pi_0
            )
    ...
    def predictive_likelihoods(
        self, test_data, forecast_horizons, num_procs=None, **kwargs
    ):
        ...
        if not num_procs:
            ...
            for k in forecast_horizons:
                step = k - prev_k
                cmaxes = cmaxes[:-step]
                scaled_alphal = scaled_alphal[:-step].dot(
                    np.linalg.matrix_power(s.trans_matrix, step)
                )
    ...
    def num_parameters(self):
        return (
            sum(o.num_parameters() for o in self.obs_distns)
            + self.num_states ** 2
            - self.num_states
        )
    ...
    def state_usages(self):
        if len(self.states_list) > 0:
            state_usages = sum(
                np.bincount(s.stateseq, minlength=self.num_states)
                for s in self.states_list
            )
    ...
    def plot(self, fig=None, plot_slice=slice(None), update=False, draw=True):
        ...
        sp1_artists = self.plot_observations(
            feature_ax, plot_slice=plot_slice, update=update
        )

        assert len(stateseq_axs) == len(self.states_list)
        sp2_artists = [
            artist
            for s, ax, data in zip(self.states_list, stateseq_axs, self.datas)
            for artist in self.plot_stateseq(
                s, ax, plot_slice, update=update, draw=False
            )
        ]
    ...
    def _get_axes(self, fig):
        ...
        if hasattr(fig, "_feature_ax") and hasattr(fig, "_stateseq_axs"):
            ...
            if len(self.states_list) <= 2:
                ...
                stateseq_axs = [
                    plt.subplot(gs[sz + idx]) for idx in range(len(self.states_list))
                ]
            else:
                gs = GridSpec(1, 2)
                sgs = GridSpecFromSubplotSpec(
                    len(self.states_list), 1, subplot_spec=gs[1]
                )

                feature_ax = plt.subplot(gs[0])
                stateseq_axs = [
                    plt.subplot(sgs[idx]) for idx in range(len(self.states_list))
                ]
    ...
    def plot_observations(
        self, ax=None, color=None, plot_slice=slice(None), update=False
    ):
        ax = ax if ax else plt.gca()
        state_colors = self._get_colors(color)
        scatter_artists = self._plot_2d_data_scatter(
            ax, state_colors, plot_slice, update
        )
        param_artists = self._plot_2d_obs_params(ax, state_colors, update)
        return scatter_artists + param_artists

    def _plot_2d_data_scatter(
        self, ax=None, state_colors=None, plot_slice=slice(None), update=False
    ):
        ...
        for state, (o, w) in enumerate(zip(self.obs_distns, usages)):
            ...
            artists.extend(
                o.plot(
                    ...
                    label="%d" % state,
                    alpha=min(0.25, 1.0 - (1.0 - w) ** 2) / 0.25,
                    ax=ax,
                    update=update,
                    draw=False,
                )
    ...
    def _get_colors(self, color=None, scalars=False, color_method=None):
        ...
        if color is None:
            ...
            if color_method == "usage":
                ...
                used_states = sorted(
                    self.used_states, key=lambda x: freqs[x], reverse=True
                )
            ...
            unused_states = [
                idx for idx in range(self.num_states) if idx not in used_states
            ]

            colorseq = np.random.RandomState(0).permutation(
                np.linspace(0, 1, self.num_states)
            )
            colors = dict(
                (idx, v if scalars else cmap(v))
                for idx, v in zip(used_states, colorseq)
            )

            for state in unused_states:
                colors[state] = cmap(1.0)
    ...
    def plot_stateseq(
        self, s, ax=None, plot_slice=slice(None), update=False, draw=True
    ):
        ...
        data_values_artist = self._plot_stateseq_data_values(
            s, ax, state_colors, plot_slice, update
        )
    ...
    def _plot_stateseq_data_values(self, s, ax, state_colors, plot_slice, update):
        ...
        colorseq = np.tile(
            np.array([state_colors[state] for state in stateseq[:-1]]), data.shape[1]
        )
        ...
        if update and hasattr(s, "_data_lc"):
            ...
            segments = np.vstack(
                [
                    AR_striding(
                        np.hstack((ts[:, None], scalarseq[:, None])), 1
                    ).reshape(-1, 2, 2)
    ...
    def _joblib_resample_states(self, states_list, num_procs):
        ...
        if len(states_list) > 0:
            joblib_args = list_split(
                [self._get_joblib_pair(s) for s in states_list], num_procs
            )
...
class _HMMSVI(_HMMBase, ModelMeanFieldSVI):
    ...
    def _meanfield_sgdstep_trans_distn(self, mb_states_list, prob, stepsize):
        self.trans_distn.meanfield_sgdstep(
            [s.expected_transcounts for s in mb_states_list], prob, stepsize
        )

    def _meanfield_sgdstep_init_state_distn(self, mb_states_list, prob, stepsize):
        self.init_state_distn.meanfield_sgdstep(
            [s.expected_states[0] for s in mb_states_list], prob, stepsize
        )
...
class _HMMEM(_HMMBase, ModelEM):
    ...
    def BIC(self, data=None):
        ...
        if data is None:
            return -2 * sum(
                self.log_likelihood(s.data).sum() for s in self.states_list
            ) + self.num_parameters() * np.log(
                sum(s.data.shape[0] for s in self.states_list)
            )
        else:
            return -2 * self.log_likelihood(data) + self.num_parameters() * np.log(
                data.shape[0]
            )
...
class _HMMViterbiEM(_HMMBase, ModelMAPEM):
    ...
    def _Viterbi_M_step_obs_distns(self):
        for state, distn in enumerate(self.obs_distns):
            distn.max_likelihood(
                [s.data[s.stateseq == state] for s in self.states_list]
            )
    ...
    def _get_mb_states_list(self, minibatch, changepoints=None, **kwargs):
        if changepoints is not None:
            if not isinstance(minibatch, (list, tuple)):
                ...
                assert isinstance(changepoints, list) and isinstance(
                    changepoints[0], tuple
                )
                minibatch = [minibatch]
                changepoints = [changepoints]
            else:
                assert (
                    isinstance(changepoints, (list, tuple))
        ...
        changepoints = (
            changepoints if changepoints is not None else [None] * len(minibatch)
        )
    ...
    def log_likelihood(self, data=None, changepoints=None, **kwargs):
        if data is not None:
            if isinstance(data, np.ndarray):
                ...
                self.add_data(
                    data=data, changepoints=changepoints, generate=False, **kwargs
                )
                return self.states_list.pop().log_likelihood()
            else:
                assert isinstance(data, list) and (
                    changepoints is None
                    or isinstance(changepoints, list)
                    and len(changepoints) == len(data)
                )
                changepoints = (
                    changepoints if changepoints is not None else [None] * len(data)
                )
...
class _HMMParallelTempering(_HMMBase, ModelParallelTempering):
    ...
    def temperature(self):
        return self._temperature if hasattr(self, "_temperature") else 1.0
    ...
    def resample_dur_distns(self):
        for state, distn in enumerate(self.dur_distns):
            distn.resample_with_censoring_and_truncation(
                data=[
                    s.durations_censored[s.untrunc_slice][
                        s.stateseq_norep[s.untrunc_slice] == state
                    ]
                    for s in self.states_list
                ],
                censored_data=[
                    s.durations_censored[s.trunc_slice][
                        s.stateseq_norep[s.trunc_slice] == state
                    ]
                    for s in self.states_list
                ],
            )
...
class _HSMMSVI(_HSMMBase, _HMMSVI):
    def _meanfield_sgdstep_parameters(self, mb_states_list, prob, stepsize):
        super(_HSMMSVI, self)._meanfield_sgdstep_parameters(
            mb_states_list, prob, stepsize
        )
...
class HSMMIntNegBin(
    _HSMMGibbsSampling, _HSMMMeanField, _HSMMSVI, _HSMMViterbiEM, _HSMMParallelTempering
):
    _trans_class = transitions.HSMMTransitions
    _trans_conc_class = transitions.HSMMTransitionsConc
    ...
    def predictive_likelihoods(self, test_data, forecast_horizons, **kwargs):
        ...
        for k in forecast_horizons:
            ...
            cmaxes = cmaxes[:-step]
            scaled_alphal = scaled_alphal[:-step].dot(
                np.linalg.matrix_power(s.hmm_trans_matrix, step)
            )
...
class HSMMIntNegBinVariant(
    _HSMMGibbsSampling, _HSMMINBEMMixin, _HSMMViterbiEM, _HSMMParallelTempering
):
    _trans_class = transitions.HSMMTransitions
    _trans_conc_class = transitions.HSMMTransitionsConc
...
class WeakLimitHDPHSMMPossibleChangepoints(
    _HSMMPossibleChangepointsMixin, WeakLimitHDPHSMM
):
    pass


class WeakLimitHDPHSMMDelayedIntNegBin(
    ...
    def __init__(self, dur_distns, delay=0, **kwargs):
        for d in dur_distns:
            d.delay = delay
        super(WeakLimitHDPHSMMDelayedIntNegBin, self).__init__(
...
class WeakLimitHDPHSMMTruncatedIntNegBin(_WeakLimitHDPMixin, HSMMIntNegBin):
    ...
    def __init__(self, dur_distns, delay=0, **kwargs):
        for d in dur_distns:
            d.delay = delay
        super(WeakLimitHDPHSMMTruncatedIntNegBin, self).__init__(
            dur_distns=dur_distns, **kwargs
        )
...
class _SeparateTransMixin(object):
    def __init__(self, *args, **kwargs):
        ...
        self.init_state_distns = collections.defaultdict(
            make_factory(self.init_state_distn)
        )
    ...
    def swap_sample_with(self, other):
        ...
        self.init_state_distns, other.init_state_distns = (
            other.init_state_distns,
            self.init_state_distns,
        )
        for d1, d2 in zip(
            self.init_state_distns.values(), other.init_state_distns.values()
        ):
            d1.model = self
            d2.model = other
    ...
    def meanfield_update_trans_distn(self):
        for group_id, trans_distn in iteritems(self.trans_distns):
            states_list = [
                s for s in self.states_list if hash(s.group_id) == hash(group_id)
            ]
            if len(states_list) > 0:
                trans_distn.meanfieldupdate(
                    [s.expected_transcounts for s in states_list]
                )
    ...
    def meanfield_update_init_state_distn(self):
        for group_id, init_state_distn in iteritems(self.init_state_distns):
            ...
            if len(states_list) > 0:
                init_state_distn.meanfieldupdate(
                    [s.expected_states[0] for s in states_list]
                )
    ...
    def _vlb(self):
        ...
        vlb += sum(
            trans_distn.get_vlb() for trans_distn in itervalues(self.trans_distns)
        )
        vlb += sum(
            init_state_distn.get_vlb()
...
class HSMMPossibleChangepointsSeparateTrans(
    _SeparateTransMixin, HSMMPossibleChangepoints
):
...
class WeakLimitHDPHSMMPossibleChangepointsSeparateTrans(
    _SeparateTransMixin, WeakLimitHDPHSMMPossibleChangepoints
):
    _states_class = hsmm_states.HSMMStatesPossibleChangepointsSeparateTrans
...
class WeakLimitHDPHSMMIntNegBinSeparateTrans(
    _SeparateTransMixin, WeakLimitHDPHSMMIntNegBin
):
...
class WeakLimitHDPHSMMDelayedIntNegBinSeparateTrans(
    _SeparateTransMixin, WeakLimitHDPHSMMDelayedIntNegBin
):
    _states_class = (
        hsmm_inb_states.HSMMStatesDelayedIntegerNegativeBinomialSeparateTrans
    )
...
class WeakLimitHDPHSMMTruncatedIntNegBinSeparateTrans(
    _SeparateTransMixin, WeakLimitHDPHSMMTruncatedIntNegBin
):
    _states_class = (
        hsmm_inb_states.HSMMStatesTruncatedIntegerNegativeBinomialSeparateTrans
    )
