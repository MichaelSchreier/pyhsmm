...
from pyhsmm.basic.distributions import (
...
class _HMMTransitionsBase(object):
    def __init__(self, num_states=None, alpha=None, alphav=None, trans_matrix=None):
        ...
        if trans_matrix is not None:
            self._row_distns = [
                Multinomial(alpha_0=alpha, K=self.N, alphav_0=alphav, weights=row)
    ...
    def _count_transitions(self, stateseqs):
        assert isinstance(stateseqs, list) and all(
            isinstance(s, np.ndarray) for s in stateseqs
        )
        return sum(
            (count_transitions(s, num_states=self.N) for s in stateseqs),
...
class _HMMTransitionsGibbs(_HMMTransitionsBase):
    def resample(self, stateseqs=[], trans_counts=None):
        trans_counts = (
            self._count_transitions(stateseqs) if trans_counts is None else trans_counts
        )
...
class _HMMTransitionsMaxLikelihood(_HMMTransitionsBase):
    def max_likelihood(self, stateseqs=None, expected_transcounts=None):
        trans_counts = (
            sum(expected_transcounts)
            if stateseqs is None
        ...
        trans_matrix[trans_matrix.sum(1) == 0] = 1.0 / trans_matrix.shape[0]
...
class _HMMTransitionsMeanField(_HMMTransitionsBase):
    ...
    def exp_expected_log_trans_matrix(self):
        return np.exp(
            np.array([distn.expected_log_likelihood() for distn in self._row_distns])
        )
...
class _HSMMTransitionsGibbs(_HSMMTransitionsBase, _HMMTransitionsGibbs):
    ...
    def _count_transitions(self, stateseqs):
        ...
        if trans_counts.sum() > 0:
            ...
            self_trans = [
                np.random.geometric(1 - A_ii, size=n).sum() if n > 0 else 0
...
class _HSMMTransitionsMaxLikelihood(_HSMMTransitionsBase, _HMMTransitionsMaxLikelihood):
    def max_likelihood(self, stateseqs=None, expected_transcounts=None):
        ...
        # all-zero rows get set to uniform
        trans_matrix[trans_matrix.sum(1) == 0] = 1.0 / (trans_matrix.shape[0] - 1)
        trans_matrix.flat[:: trans_matrix.shape[0] + 1] = 0.0

        self.trans_matrix = trans_matrix
        assert np.allclose(0.0, np.diag(self.trans_matrix))
        assert np.allclose(1.0, self.trans_matrix.sum(1))
...
class _WeakLimitHDPHMMTransitionsGibbs(
    _WeakLimitHDPHMMTransitionsBase, _HMMTransitionsGibbs
):
...
class WeakLimitHDPHMMTransitions(
    _WeakLimitHDPHMMTransitionsGibbs, _HMMTransitionsMaxLikelihood
):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.beta_obj = MultinomialAndConcentration(
            a_0=gamma_a_0, b_0=gamma_b_0, K=self.N, weights=beta
        )
...
class _WeakLimitStickyHDPHMMTransitionsGibbs(
    ...
    def _get_m(self, trans_counts):
        ...
        if ms.sum() > 0:
            ...
            newms.flat[:: ms.shape[0] + 1][indices] = np.array(
                np.random.binomial(
                    ms.flat[:: ms.shape[0] + 1][indices],
                    self.beta[indices]
                    * self.alpha
                    / (self.beta[indices] * self.alpha + self.kappa),
                ),
                dtype=np.int32,
            )
        return newms
...
class WeakLimitStickyHDPHMMTransitionsConc(_WeakLimitStickyHDPHMMTransitionsConcGibbs):
    pass
...
class _DATruncHDPHMMTransitionsBase(_HMMTransitionsBase):
    ...
    def __init__(self, gamma, alpha, num_states, beta=None, trans_matrix=None):
        ...
        betafull = np.concatenate(((beta, (1.0 - beta.sum(),))))
    ...
    def _sample_GEM(gamma, K):
        v = np.random.beta(1.0, gamma, size=K)
        return v * np.concatenate(((1.0,), np.cumprod(1.0 - v[:-1])))
    ...
    def beta(self, beta):
        self._beta = beta
        self.alphav = self._alpha * np.concatenate((beta, (1.0 - beta.sum(),)))

    @property
    def exp_expected_log_trans_matrix(self):
        return (
            super(_DATruncHDPHMMTransitionsBase, self)
            .exp_expected_log_trans_matrix[:, :-1]
            .copy()
        )
...
class _DATruncHDPHMMTransitionsSVI(_DATruncHDPHMMTransitionsBase, _HMMTransitionsSVI):
    ...
    def meanfield_sgdstep(self, expected_transcounts, prob, stepsize):
        ...
        assert (self.beta >= 0.0).all() and self.beta.sum() < 1
        return self
    ...
    def _feasible_step(pt, grad, stepsize):
        ...
        def feas(pt):
            return (pt > 0.0).all() and pt.sum() < 1.0
    ...
    def _beta_gradient(self):
        return self._grad_log_p_beta(self.beta, self.gamma) + sum(
            self._grad_E_log_p_pi_given_beta(self.beta, self._alpha, distn._alpha_mf)
            for distn in self._row_distns
        )
    ...
    def _grad_log_p_beta(beta, alpha):
        ...
        return -(alpha - 1) * rcumsum(1.0 / (1 - cumsum(beta))) + 2 * rcumsum(
            1.0 / (1 - cumsum(beta, strict=True)), strict=True
        )

    def _grad_E_log_p_pi_given_beta(self, beta, gamma, alphatildes):
        # NOTE: switched argument name gamma <-> alpha
        retval = gamma * (
            digamma(alphatildes[:-1]) - digamma(alphatildes[-1])
        ) - gamma * (digamma(gamma * beta) - digamma(gamma))
        return retval

    def get_vlb(self):
        return super(_DATruncHDPHMMTransitionsSVI, self).get_vlb() + self._beta_vlb()

    def _beta_vlb(self):
        return (
            np.log(self.beta).sum()
            + self.gamma * np.log(1 - cumsum(self.beta)).sum()
...
class _DATruncHDPHSMMTransitionsSVI(_DATruncHDPHMMTransitionsSVI, _HSMMTransitionsSVI):
    ...
    def _beta_gradient(self):
        return self._grad_log_p_beta(self.beta, self.gamma) + sum(
            self._zero_ith_component(
                self._grad_E_log_p_pi_given_beta(
                    self.beta, self._alpha, distn._alpha_mf
                ),
                i,
            )
