...
class HSMMStatesPython(_StatesBase):
    ...
    def durations(self):
        ...
        if self.left_censoring:
            durs[0] = self.dur_distns[self.stateseq_norep[0]].rvs_given_greater_than(
                durs[0] - 1
            )
        if self.right_censoring:
            durs[-1] = self.dur_distns[self.stateseq_norep[-1]].rvs_given_greater_than(
                durs[-1] - 1
            )
        return durs
    ...
    def untrunc_slice(self):
        return slice(
            1 if self.left_censoring else 0, -1 if self.right_censoring else None
        )
    ...
    def generate_states(self):
        ...
        while idx < self.T:
            ...
            stateseq[
                idx : idx + duration
            ] = state  # this can run off the end, that's okay
    ...
    def messages_forwards(self):
        alphal, alphastarl, _ = hsmm_messages_forwards_log(
            ...
            np.empty((self.T, self.num_states)),
            np.empty((self.T, self.num_states)),
        )
    ...
    def cumulative_obs_potentials(self, t):
        ...
        return np.cumsum(self.aBl[t:stop], axis=0), 0.0
    ...
    def dur_survival_potentials(self, t):
        return (
            self.aDsl[self.T - t - 1]
            if (self.trunc is None or self.T - t > self.trunc)
            else -np.inf
        )
    ...
    def reverse_dur_survival_potentials(self, t):
        ...
        return self.aDsl[t] if (self.trunc is None or t + 1 < self.trunc) else -np.inf
    ...
    def mf_cumulative_obs_potentials(self, t):
        stop = None if self.trunc is None else min(self.T, t + self.trunc)
        return np.cumsum(self.mf_aBl[t:stop], axis=0), 0.0
    ...
    def mf_reverse_dur_survival_potentials(self, t):
        # NOTE: untested, unused without left-censoring
        return (
            self.mf_aDsl[t] if (self.trunc is None or t + 1 < self.trunc) else -np.inf
        )
    ...
    def mf_Viterbi(self):
        self.stateseq = hsmm_maximizing_assignment(
            self.num_states,
            self.T,
            self.mf_trans_potentials,
            np.log(self.mf_pi_0),
            self.mf_cumulative_obs_potentials,
            self.mf_reverse_cumulative_obs_potentials,
            self.mf_dur_potentials,
            self.mf_dur_survival_potentials,
        )
    ...
    def meanfieldupdate(self):
        ...
        self.all_expected_stats = self._expected_statistics(
            ...
            self.mf_reverse_cumulative_obs_potentials,
            self.mf_dur_potentials,
            self.mf_reverse_dur_potentials,
            self.mf_dur_survival_potentials,
            self.mf_reverse_dur_survival_potentials,
        )
    ...
    def _expected_statistics(
        ...
        expected_transitions = self._expected_transitions(
            alphal, betastarl, trans_potentials, normalizer
        )  # TODO assumes homog trans

        expected_durations = self._expected_durations(
            dur_potentials, cumulative_obs_potentials, alphastarl, betal, normalizer
        )
    ...
    def _expected_states(self, alphal, betal, alphastarl, betastarl, normalizer):
        ...
        assert gamma.min() > 0.0 - 1e-3 and gamma.max() < 1.0 + 1e-3
        assert gammastar.min() > 0.0 - 1e-3 and gammastar.max() < 1.0 + 1e-3

        expected_states = (
            gammastar - np.vstack((np.zeros(gamma.shape[1]), gamma[:-1]))
        ).cumsum(0)

        assert not np.isnan(expected_states).any()
        assert expected_states.min() > 0.0 - 1e-3 and expected_states.max() < 1 + 1e-3
        assert np.allclose(expected_states.sum(1), 1.0, atol=1e-2)

        expected_states = np.maximum(0.0, expected_states)
    ...
    def _expected_durations(
        self, dur_potentials, cumulative_obs_potentials, alphastarl, betal, normalizer
    ):
...
class HSMMStatesEigen(HSMMStatesPython):
    ...
    def messages_backwards(self):
        ...
        betal, betastarl = messages_backwards_log(
            np.maximum(self.trans_matrix, 1e-50),
            self.aBl,
            np.maximum(self.aDl, -1000000),
            self.aDsl,
            np.empty_like(self.aBl),
            np.empty_like(self.aBl),
            self.right_censoring,
            self.trunc if self.trunc is not None else self.T,
        )
...
class GeoHSMMStates(HSMMStatesPython):
    ...
    def hmm_trans_matrix(self):
        ...
        assert np.allclose(1.0, A.sum(1))
...
class _PossibleChangepointsMixin(
    hmm_states._PossibleChangepointsMixin, HSMMStatesPython
):
    @property
    def stateseq(self):
...
class HSMMStatesPossibleChangepoints(_PossibleChangepointsMixin, HSMMStatesPython):
    ...
    def dur_potentials(self, tblock):
        possible_durations = (
            self.segmentlens[tblock:].cumsum()[: self.trunc].astype("int32")
        )
    ...
    def mf_cumulative_obs_potentials(self, tblock):
        return self.mf_aBl[tblock:].cumsum(0)[: self.trunc], 0.0
    ...
    def _expected_durations(
        ...
        for tblock in range(self.Tblock):
            ...
            logpmfs[possible_durations - 1] = np.logaddexp(
                dur_potentials(tblock)
                + alphastarl[tblock]
                + betal[
                    tblock : tblock + self.trunc if self.trunc is not None else None
                ]
                + cB
                - (offset + normalizer),
...
class HSMMStatesPossibleChangepointsSeparateTrans(
    _SeparateTransMixin, HSMMStatesPossibleChangepoints
):
    pass
...
class DiagGaussStates(HSMMStatesPossibleChangepointsSeparateTrans):
    ...
    def aBl(self):
        if self._aBl is None:
            ...
            aBl = (
                np.einsum("td,td,nd->tn", self.data, self.data, Js)
                - np.einsum("td,nd,nd->tn", self.data, 2 * mus, Js)
            ) + (mus ** 2 * Js - 1.0 / 2 * np.log(2 * np.pi * sigmas)).sum(1)
            aBl[np.isnan(aBl).any(1)] = 0.0
...
class DiagGaussGMMStates(HSMMStatesPossibleChangepointsSeparateTrans):
    ...
    def aBl_einsum(self):
        if self._aBBl is None:
            sigmas = np.array(
                [[c.sigmas for c in d.components] for d in self.obs_distns]
            )
            Js = -1.0 / (2 * sigmas)
            ...
            all_likes = np.einsum(
                "td,td,nkd->tnk", self.data, self.data, Js
            ) - np.einsum("td,nkd,nkd->tnk", self.data, 2 * mus, Js)
            all_likes += (mus ** 2 * Js - 1.0 / 2 * np.log(2 * np.pi * sigmas)).sum(2)
...
def hsmm_messages_backwards_log(
    ...
    for t in range(T - 1, -1, -1):
        ...
        if right_censoring:
            np.logaddexp(
                betastarl[t],
                cB[-1] - offset + dur_survival_potentials(t),
                out=betastarl[t],
            )
        betal[t - 1] = logsumexp(betastarl[t] + trans_potentials(t - 1), axis=1)
    betal[-1] = 0.0  # overwritten on last iteration
...
def hsmm_messages_forwards_log(
    ...
    for t in range(T - 1):
        ...
        alphal[t] = logsumexp(
            alphastarl[t + 1 - cB.shape[0] : t + 1] + cB + reverse_dur_potentials(t),
            axis=0,
        )
        if left_censoring:
            raise NotImplementedError
        alphastarl[t + 1] = logsumexp(alphal[t][:, na] + trans_potential(t), axis=0)
    t = T - 1
    cB = reverse_cumulative_obs_potentials(t)
    alphal[t] = logsumexp(
