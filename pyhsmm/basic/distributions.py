...
class _StartAtOneMixin(object):
    ...
    def expected_log_likelihood(self, x, *args, **kwargs):
        return super(_StartAtOneMixin, self).expected_log_likelihood(
            x - 1, *args, **kwargs
        )
    ...
    def resample(self, data=[], *args, **kwargs):
        if isinstance(data, np.ndarray):
            ...
            return super(_StartAtOneMixin, self).resample(
                [d - 1 for d in data], *args, **kwargs
            )
    ...
    def meanfield_sgdstep(self, data, weights, prob, stepsize):
        if isinstance(data, np.ndarray):
            return super(_StartAtOneMixin, self).meanfield_sgdstep(
                data - 1, weights=weights, prob=prob, stepsize=stepsize
            )
        else:
            return super(_StartAtOneMixin, self).meanfield_sgdstep(
                [d - 1 for d in data], weights=weights, prob=prob, stepsize=stepsize
            )
...
class NegativeBinomialDuration(
    _StartAtOneMixin, NegativeBinomial, DurationDistribution
):
    pass


class NegativeBinomialFixedRDuration(
    _StartAtOneMixin, NegativeBinomialFixedR, DurationDistribution
):
    pass


class NegativeBinomialIntegerRDuration(
    _StartAtOneMixin, NegativeBinomialIntegerR, DurationDistribution
):
    pass


class NegativeBinomialIntegerR2Duration(
    _StartAtOneMixin, NegativeBinomialIntegerR2, DurationDistribution
):
    pass


class NegativeBinomialFixedRVariantDuration(
    NegativeBinomialFixedRVariant, DurationDistribution
):
    pass


class NegativeBinomialIntegerRVariantDuration(
    NegativeBinomialIntegerRVariant, DurationDistribution
):
    pass
...
class Delay(DurationDistribution):
    ...
    def resample(self, data=[], *args, **kwargs):
        if isinstance(data, np.ndarray):
            ...
            return self.dur_distn.resample(
                [d - self.delay for d in data], *args, **kwargs
            )
