...
class DurationDistribution(with_metaclass(abc.ABCMeta, Distribution)):
    ...
    def _uncensor_data(self, censored_data):
        ...
        if len(censored_data) > 0:
            if not isinstance(censored_data, list):
                filled_in = np.asarray(
                    [self.rvs_given_greater_than(x - 1) for x in censored_data]
                )
            else:
                filled_in = np.asarray(
                    [
                        self.rvs_given_greater_than(x - 1)
                        for xx in censored_data
                        for x in xx
                    ]
                )
        else:
    ...
    def resample_with_censoring_and_truncation(
        self, data=[], censored_data=[], left_truncation_level=None
    ):
        filled_in = self._uncensor_data(censored_data)
        ...
        if left_truncation_level is not None and left_truncation_level > 1:
            ...
            rejected_observations = (
                self.rvs_given_less_than(left_truncation_level, num_rejected)
                if num_rejected > 0
                else []
            )
        else:
