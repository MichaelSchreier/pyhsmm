...
def mask_data(data):
    return np.ma.masked_array(
        np.nan_to_num(data), np.isnan(data), fill_value=0.0, hard_mask=True
    )
...
def sample_discrete(distn, size=[], dtype=np.int32):
    ...
    return np.sum(
        np.array(random(size))[..., na] * cumvals[-1] > cumvals, axis=-1, dtype=dtype
    )
...
def sample_discrete_from_log(p_log, axis=0, dtype=np.int32):
    ...
    thesize[axis] = 1
    randvals = random(size=thesize) * np.reshape(
...
def sample_crp_tablecounts(concentration, customers, colweights):
    ...
    for (i, j), n in np.ndenumerate(customers):
        w = colweights[j]
        for k in range(n):
            m[i, j] += randseq[starts[i, j] + k] < (concentration * w) / (
                k + concentration * w
            )
...
def invwishart_entropy(sigma, nu, chol=None):
    ...
    Elogdetlmbda = (
        special.digamma((nu - np.arange(D)) / 2).sum()
        + D * np.log(2)
        - 2 * np.log(chol.diagonal()).sum()
    )
    return (
        invwishart_log_partitionfunction(sigma, nu, chol)
        - (nu - D - 1) / 2 * Elogdetlmbda
        + nu * D / 2
    )
...
def multivariate_t_loglik(y, nu, mu, lmbda):
    ...
    return (
        scipy.special.gammaln((nu + d) / 2.0)
        - scipy.special.gammaln(nu / 2.0)
        - (d / 2.0) * np.log(nu * np.pi)
        - np.log(L.diagonal()).sum()
        - (nu + d) / 2.0 * np.log1p(1.0 / nu * inner1d(ys.T, ys.T))
    )
...
def beta_predictive(priorcounts, newcounts):
    ...
    numer = scipy.special.gammaln(
        np.array([nsuc + prior_nsuc, nfail + prior_nfail, prior_nsuc + prior_nfail])
    ).sum()
    denom = scipy.special.gammaln(
        np.array([prior_nsuc, prior_nfail, prior_nsuc + prior_nfail + nsuc + nfail])
    ).sum()
    return numer - denom
...
def two_sample_t_statistic(pop1, pop2):
    ...
    t = (pop1.mean(0) - pop2.mean(0)) / np.sqrt(
        pop1.var(0) / pop1.shape[0] + pop2.var(0) / pop2.shape[0]
    )
...
def f_statistic(pop1, pop2):  # TODO test
    ...
    n1, n2 = (
        np.where(var1 >= var2, pop1.shape[0], pop2.shape[0]),
