...
def plot_gaussian_2D(
    mu, lmbda, color="b", centermarker=True, label="", alpha=1.0, ax=None, artists=None
):
    """
    ...
    if artists is None:
        point = (
            ax.scatter([mu[0]], [mu[1]], marker="D", color=color, s=4, alpha=alpha)
            if centermarker
            else None
        )
        (line,) = ax.plot(
...
def subplot_gridsize(num):
    return sorted(
        min(
            [
                (x, int(np.ceil(num / x)))
                for x in range(1, int(np.floor(np.sqrt(num))) + 1)
            ],
            key=sum,
        )
    )
