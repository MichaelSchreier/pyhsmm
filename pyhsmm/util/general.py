...
def scoreatpercentile(data, per, axis=0):
    ...
    idx = per / 100.0 * (data.shape[axis] - 1)
    ...
    if idx % 1 == 0:
        ...
        return (
            lowerweight
            * a[[slice(None) if ii != axis else idx for ii in range(a.ndim)]]
            + upperweight
            * a[[slice(None) if ii != axis else idx + 1 for ii in range(a.ndim)]]
        )
...
def sgd_passes(tau, kappa, datalist, minibatchsize=1, npasses=1):
    ...
    for superitr in range(npasses):
        if minibatchsize == 1:
            ...
            minibatch_indices = np.array_split(
                np.random.permutation(N), N / minibatchsize
            )
...
def relabel_by_usage(labelss, return_mapping=False, N=None):
    ...
    usages = sum(
        np.bincount(l[~np.isnan(l)].astype("int32"), minlength=N) for l in labelss
    )
